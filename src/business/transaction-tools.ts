...
export const transColumns: { name: string; valueFn: ValueExtractor; format: string | null }[] = [
    ...
    transColumn("amount", (t) => t.trn.to_base),
    ...
    transColumn("Plaid:TransactionCategory", (t) => {
        ...
        const s = d.startsWith(p) ? d.slice(p.length) : d;
        return JJ(p, s);
    }),
    ...
    transColumn("lunchId", (t) => t.trn.id),
...
function transColumn(name: string, valueFn: ValueExtractor, format: string | null = null) {
    return {
        name,
        valueFn,
        format,
...
function transTagColumn(tagGroupName: string) {
    return {
        name: `Tags:${tagGroupName}`,
        valueFn: (t: Transaction) => J(t.tag.get(tagGroupName), TagListSeparator),
        format: null,
    };
}
